...
actor class _ManagerCanister() {
  ...
  public shared ({ caller }) func updateUsername(dto : UserCommands.SetUsername) : async Result.Result<(), MopsEnums.Error> {
    assert not Principal.isAnonymous(caller);
    let backendPrincipalId = Principal.toText(caller);
    assert backendPrincipalId == Environment.BACKEND_CANISTER_ID;
  ...
  private func mergeManagerUsername(manager : AppTypes.Manager, username : Text) : AppTypes.Manager {
    return {
      principalId = manager.principalId;
      username = username;
      termsAccepted = manager.termsAccepted;
  ...
  public shared ({ caller }) func updateProfilePicture(dto : UserCommands.SetProfilePicture) : async Result.Result<(), MopsEnums.Error> {
    assert not Principal.isAnonymous(caller);
    let backendPrincipalId = Principal.toText(caller);
    assert backendPrincipalId == Environment.BACKEND_CANISTER_ID;
  ...
  private func mergeManagerProfilePicture(manager : AppTypes.Manager, profilePicture : Blob, extension : Text) : AppTypes.Manager {
    return {
      principalId = manager.principalId;
      username = manager.username;
      termsAccepted = manager.termsAccepted;
  ...
  private func mergeExistingHistory(existingHistory : List.List<AppTypes.FantasyTeamSeason>, fantasyTeamSnapshot : UserQueries.FantasyTeamSnapshot, seasonId : FootballTypes.SeasonId, gameweek : FootballTypes.GameweekNumber) : List.List<AppTypes.FantasyTeamSeason> {
