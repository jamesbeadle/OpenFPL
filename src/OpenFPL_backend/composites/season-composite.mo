...
module {
  ...
  public class SeasonComposite() {
    ...
    public func fixIncorrectData(){
      //update the events on the fixtures
      let seasonBuffer = Buffer.fromArray<T.Season>([]);
        ...
        if(season.id == 1){
          ...
          for(fixture in Iter.fromList(season.fixtures)){
            if(fixture.gameweek == 1){
              if(fixture.id == 1){
                let playerEventsBuffer = Buffer.fromArray<T.PlayerEventData>([]);
                var addedOnce = false;
                for(playerEvent in Iter.fromList(fixture.events)){
                  if(playerEvent.playerId == 394){
                    if(not addedOnce){
                      playerEventsBuffer.add(playerEvent);  
                      addedOnce := true;
                    } else {
                      playerEventsBuffer.add({
                        clubId = playerEvent.clubId;
                        eventEndMinute = playerEvent.eventEndMinute;
                        eventStartMinute = playerEvent.eventStartMinute;
                        eventType = playerEvent.eventType;
                        fixtureId = playerEvent.fixtureId;
                        playerId = 395
                      });
                    }
                  } else {
                ...
                fixturesBuffer.add({
                  awayClubId = fixture.awayClubId;
                  awayGoals = fixture.awayGoals;
                  events = List.fromArray(Buffer.toArray(playerEventsBuffer));
                  gameweek = fixture.gameweek;
                  highestScoringPlayerId = fixture.highestScoringPlayerId;
                  homeClubId = fixture.homeClubId;
                  homeGoals = fixture.homeGoals;
