...
module {
  ...
  public func decode(text : Text) : Result.Result<[Nat8], MopsEnums.Error> {
    let next = text.chars().next;
    func parse() : Result.Result<Nat8, MopsEnums.Error> {
      Option.get<Result.Result<Nat8, MopsEnums.Error>>(
        do ? {
          let c1 = next()!;
          let c2 = next()!;
          Result.chain<Nat8, Nat8, MopsEnums.Error>(
            decodeW4(c1),
            func(x1) {
              Result.chain<Nat8, Nat8, MopsEnums.Error>(
                decodeW4(c2),
  ...
  private func decodeW4(char : Char) : Result.Result<Nat8, MopsEnums.Error> {
