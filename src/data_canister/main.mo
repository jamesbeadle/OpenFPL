...
/*
      let openfpl_backend_canister = actor ("bboqb-jiaaa-aaaal-qb6ea-cai") : actor {
        getPlayers : () -> async [T.OldPlayerDTO];
        getClubs : () -> async [T.OldClubDTO];
        getSeasons : () -> async [T.OldSeasonDTO];
        getFixtures : () -> async [T.OldFixtureDTO];
      };

      let seasons = await openfpl_backend_canister.getSeasons();
      let fixtures = await openfpl_backend_canister.getFixtures();
      let clubs = await openfpl_backend_canister.getClubs();
      let players = await openfpl_backend_canister.getPlayers();
      
      let fixturesList = List.fromArray(fixtures);
*/
      /*
      leagueSeasons := [(1, Array.map<T.OldSeasonDTO, T.Season>(seasons, func(season: T.OldSeasonDTO){
        return {
          fixtures = List.map<T.OldFixtureDTO, T.Fixture>(fixturesList, func(fixture: T.OldFixtureDTO){
            return {
              awayClubId = fixture.awayClubId;
              awayGoals = fixture.awayGoals;
