...
shared actor class NeuronController() = self {
  ...
  //
  public query func transform(args : T.Http.TransformArgs) : async T.Http.HttpResponsePayload {
    Loopback.transform(args);
  };
  ...
  public shared ({ caller }) func manage_neuron(cmd : T.Command) : async T.NeuronResponse {
    ...
    let request : T.ManageNeuron = {
      command = ?cmd;
      id = ?{ id = state.neuron_id };
  ...
  public shared ({ caller }) func stake_nns_neuron() : async T.NeuronResponse {
    ...
    if (state.neuron_id > 0) return #ok({
      command = ?#ClaimOrRefresh({
        refreshed_neuron_id = ?{ id = state.neuron_id };
      });
    });
    ...
    switch (await ledger_client.transfer({ memo = nonce; amount = { e8s = 100_000_000 }; /* 1 ICP */ fee = { e8s = MIN_FEE }; /* .001 ICP */ from_subaccount = null; created_at_time = null; to = to_account })) {
    ...
    switch (await governance_client.manage_neuron({ id = null; neuron_id_or_subaccount = null; command = ?#ClaimOrRefresh({ by = ?#MemoAndController({ controller = ?p; memo = nonce }) }) })) {
