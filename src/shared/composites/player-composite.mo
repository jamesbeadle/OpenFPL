...
module {
  public class PlayerComposite() {
    ...
    public func getActivePlayers(leagueId: T.FootballLeagueId) : async Result.Result<[DTOs.PlayerDTO], T.Error> {
      let data_canister = actor (NetworkEnvironmentVariables.DATA_CANISTER_ID) : actor {
        getActivePlayers : (leagueId: T.FootballLeagueId) -> async Result.Result<[DTOs.PlayerDTO], T.Error>;
      };
      return await data_canister.getActivePlayers(leagueId);
    };
