...
<Layout>
  ...
  {:else}
    <AddPlayerModal {showAddPlayer} closeAddPlayerModal={closeAddPlayerModal} />
    <div class="m-4">
              ...
              {#each gridSetup as row, idx}
                ...
                {#each row as _, colIndex (colIndex)}
                  <div class="flex items-center justify-between py-2 px-4">
                    ...
                    <div class="w-1/3">-</div>
                    <div class="w-1/6">-</div>
                    <div class="w-1/6 flex items-center"><button on:click={loadAddPlayer} class="text-xl rounded fpl-button flex items-center"><AddIcon className="w-6 h-6 p-2" /></button></div>
                  </div>
                {/each}
              {/each}
            </div>
