...
export class MVPService {
  ...
  async getMostValuableGameweekPlayers(
    ...
    try {
      ...
      if (isError(result))
        throw new Error("Failed to fetch most valuable gameweek players");
      return result.ok;
    } catch (error) {
      console.error("Error fetching fixtures: ", error);
      toastsStore.addToast({
        type: "error",
        message: "Error fetching most valuable gameweek players.",
      });
    }
  }
}
