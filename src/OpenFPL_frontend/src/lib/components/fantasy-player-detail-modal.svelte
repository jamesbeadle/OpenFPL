<script lang="ts">
  ...
  {#if showModal}
    <div class="fixed inset-0 bg-gray-900 bg-opacity-80 overflow-y-auto h-full w-full modal-backdrop" on:click={closeDetailModal}
      ...
      <div class="relative top-20 mx-auto p-5 border border-gray-700 w-96 shadow-lg rounded-md bg-panel text-white"
        ...
        <div class="flex justify-start items-center w-full">
          <svelte:component this={getFlagComponent(gameweekData.player.nationality)} class="h-20 w-20"/>
          <div class="ml-4">
            <h3 class="text-2xl mb-2">
                {(gameweekData.player.firstName != "" ? gameweekData.player.firstName.charAt(0) + "." : "") + gameweekData.player.lastName}
            </h3>
        ...
        <div class="flex justify-start items-center w-full border-t border-gray-600 text-sm">
          <p class="flex w-1/3 items-center border-r border-gray-600 justify-center pt-2">
            vs <BadgeIcon
        ...
        <div class="flex justify-between items-center p-2">
            ...
            <div class="text-sm font-medium w-1/6">{gameweekData.appearance > 0 ? gameweekData.appearance * 5 : "-"}</div>
        </div>
        ...
        <div class="flex justify-between items-center p-2">
            ...
            <div class="text-sm font-medium w-1/6">{gameweekData.yellowCards * -5}</div>
        </div>
  
        <div class="flex justify-between items-center p-2">
            <div class="text-sm font-medium w-3/6">Red Card</div>
            <div class="text-sm font-medium w-2/6">{gameweekData.redCards}</div>
            <div class="text-sm font-medium w-1/6">{gameweekData.redCards > 0 ? -20 : 0}</div>
        </div>
  
        {#if gameweekData.player.position < 2}
            <div class="flex justify-between items-center p-2">
                <div class="text-sm font-medium w-3/6">Clean Sheet</div>
                <div class="text-sm font-medium w-2/6">{gameweekData.cleanSheets}</div>
                <div class="text-sm font-medium w-1/6">{gameweekData.cleanSheetPoints}</div>
            </div>

            <div class="flex justify-between items-center p-2">
                <div class="text-sm font-medium w-3/6">Conceded</div>
                <div class="text-sm font-medium w-2/6">{gameweekData.goalsConceded}</div>
