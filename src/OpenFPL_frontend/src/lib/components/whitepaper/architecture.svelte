<div class="m-4">
  ...
  <p class="my-2">
    ...
    each users principal and username for efficient existing username checks. 
  </p>
  <p class="my-2">
    The Main Backend canister also stores a list of each unique manager canister to enable efficient grouped lookups. 
    Even with 10m users the Main Backend Canister only uses 2GB of its available storage, 
    however it is likely that the players and user indexes are refactored into their own canisters 
    after the SNS sale but before the season starts.
  </p>

  <h2 class="default-sub-header mt-4">Manager Data</h2>
  <p class="my-2">
    A snapshot of a fantasy team is around 147 bytes. So if a managers players for 100 seasons that is a 
    total history of 546 KB (100 x 38 x 147 bytes).
  ...
  <p class="my-2">
    ...
    This means the leaderboard canister can hold over 35m entries within its 4GB heap memory, 
    more than 3 times the players of the market leading game.
  </p>
</div>
