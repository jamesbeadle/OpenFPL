<script lang="ts">
  ...
  $: isSubmitDisabled = eventType < 0 || eventStartTime < 0 || eventStartTime > 90 || eventEndTime < 0 || eventEndTime > 90;
  ...
  <div class="fixed inset-0 bg-gray-900 bg-opacity-80 overflow-y-auto h-full w-full modal-backdrop"
    ...
    <div class="relative top-20 mx-auto p-5 border border-gray-700 w-96 shadow-lg rounded-md bg-panel"
      ...
      <div class="mt-3">
          ...
          <div class="flex flex-col gap-1">
            ...
            <div class="mt-1">
              ...
              <input type="number" id="startMinute" bind:value={eventStartTime}
                ...
                placeholder="Enter start minute" min="0" max="90"/>
            ...
            <div class="mt-2">
              ...
              class="bg-gray-900 w-full px-4 py-2 mt-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              placeholder="Enter end minute" min="0" max="90"/>
            </div>
          ...
          <ul class="list-disc">
            {#each $playerEventData as event, index}
              <li class="flex justify-between items-center mb-2">
                <span>{getEventTypeLabel(event.eventType)} ({event.eventStartMinute} - {event.eventEndMinute}) mins</span>
        ...
        <div class="items-center mt-3 flex space-x-4">
          <button class="fpl-button mx-4 px-4 py-2 text-white text-base font-medium rounded-md w-full shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-300" on:click={closeModal}>Done</button>
        </div>
      </div>
    </div>
  </div>
