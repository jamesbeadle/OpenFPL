<script lang="ts">
    ...
    async function confirmVote() {
      ...
      const identityActor: any = await ActorFactory.createIdentityActor(
        ...
        const { listNeurons, registerVote } = GovernanceCanister.create(identityActor);
        let neurons = await listNeurons({certified: false});
        
        neurons.forEach(element => {
            switch(vote){
                case "Yes":
                    registerVote({neuronId: element.neuronId, proposalId: proposal.id!, vote: Vote.Yes });
                    break;
                case "No":
                    registerVote({neuronId: element.neuronId, proposalId: proposal.id!, vote: Vote.No });
                    break;
            }
        });
