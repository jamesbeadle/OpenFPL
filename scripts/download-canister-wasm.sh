...
then
  COMMIT_ID=$(curl -s https://openfpl-canister-wasms.s3.amazonaws.com/latest)
...
HTTP_CODE=$(curl -sO https://openfpl-canister-wasms.s3.amazonaws.com/$COMMIT_ID/$CANISTER_NAME.wasm.gz --write-out "%{http_code}")
