...
const Fixtures = create_ssr_component(
  ($$result, $$props, $$bindings, slots) => {
    ...
    groupedFixtures = filteredFixtures.reduce((acc, fixtureWithTeams) => {
    ...
    return `<div class="container-fluid mt-4 mb-4"><div class="flex flex-col space-y-4"><div class="flex flex-col sm:flex-row gap-4 sm:gap-8"><div class="flex items-center space-x-2 ml-4"><button class="text-base sm:text-xs md:text-base rounded fpl-button px-3 sm:px-2 px-3 py-1" ${"disabled"}>&lt;
        ...
        <select class="p-2 fpl-dropdown text-xs md:text-base text-center mx-0 md:mx-2 min-w-[150px] sm:min-w-[100px]">${each(
          gameweeks,
          (gameweek) => {
            return `<option${add_attribute(
              ...
              gameweek,
              0
            )}>Gameweek ${escape(gameweek)}</option>`;
          }
        )}</select>
    ...
    <div>${each(Object.entries(groupedFixtures), ([date, fixtures]) => {
      return `<div><div class="flex items-center justify-between border border-gray-700 py-4 bg-light-gray"><h2 class="date-header ml-4 text-xs md:text-base">${escape(
        date
      )}</h2></div>
          ${each(fixtures, ({ fixture, homeTeam, awayTeam }) => {
            return `<div${add_attribute(
              "class",
              `flex items-center justify-between py-2 border-b border-gray-700  ${
                fixture.status === 0 ? "text-gray-400" : "text-white"
              }`,
              0
            )}><div class="flex items-center w-1/2 ml-4"><div class="flex w-1/2 space-x-4 justify-center"><div class="w-10 items-center justify-center"><a${add_attribute(
              "href",
              `/club?id=${fixture.homeTeamId}`,
              0
              ...
              {}
                  ...
                  <div class="w-10 items-center justify-center"><a${add_attribute(
                    "href",
                    `/club?id=${fixture.awayTeamId}`,
                    0
                  )}>${validate_component(BadgeIcon, "BadgeIcon").$$render(
              $$result,
              {
              ...
              {}
                ...
                <div class="flex w-1/2 lg:justify-center"><span class="text-sm md:text-lg ml-4 md:ml-0 text-left">${escape(
                  formatUnixTimeToTime(Number(fixture.kickOff))
                )}</span>
                </div></div>
              <div class="flex items-center space-x-10 w-1/2 lg:justify-center"><div class="flex flex-col min-w-[200px] lg:min-w-[120px] lg:min-w-[200px] text-xs md:text-base"><a${add_attribute(
                "href",
                `/club?id=${fixture.homeTeamId}`,
                0
              )}>${escape(homeTeam ? homeTeam.friendlyName : "")}</a>
                  <a${add_attribute(
                    "href",
                    `/club?id=${fixture.awayTeamId}`,
                    0
                  )}>${escape(awayTeam ? awayTeam.friendlyName : "")}</a></div>
                <div class="flex flex-col min-w-[200px] lg:min-w-[120px] lg:min-w-[200px] text-xs md:text-base"><span>${escape(
                  fixture.status === 0 ? "-" : fixture.homeGoals
                )}</span>
                  <span>${escape(
                    fixture.status === 0 ? "-" : fixture.awayGoals
                  )}</span>
                </div></div>
            </div>`;
          })}
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
    ...
    {},
    {
      default: () => {
        return `<div class="m-4"><div class="flex flex-col lg:flex-row"><div class="flex justify-start items-center text-white space-x-4 flex-grow m-4 bg-panel p-4 rounded-md"><div class="flex-grow"><p class="text-gray-300 text-xs">Gameweek</p>
          <p class="text-2xl sm:text-3xl lg:text-4xl mt-2 mb-2 font-bold">${escape(
            activeGameweek
          )}</p>
      ...
      <div class="flex flex-col lg:flex-row justify-start lg:items-center text-white space-x-0 lg:space-x-4 flex-grow m-4 bg-panel p-4 rounded-md"><div class="flex-grow mb-4 lg:mb-0"><p class="text-gray-300 text-xs">Next Game:</p>
        ...
        <div class="flex-grow mb-4 lg:mb-0"><p class="text-gray-300 text-xs mt-4 lg:mt-0">Kick Off:</p>
          <div class="flex"><p class="text-2xl sm:text-3xl lg:text-4xl mt-2 mb-2 font-bold">${escape(
            countdownDays
          ...
          <p class="text-gray-300 text-xs">${escape(
            nextFixtureDate
          )} | ${escape(nextFixtureTime)}</p></div>
        <div class="h-px bg-gray-400 w-full lg:w-px lg:h-full lg:self-stretch" style="min-height: 2px; min-width: 2px;"></div>
        <div class="flex-grow"><p class="text-gray-300 text-xs mt-4 lg:mt-0">GW ${escape(
          focusGameweek
        )} High Score
          </p>
  ...
  <div class="mx-4"><div class="bg-panel rounded-md mx-4"><ul class="flex bg-light-gray px-4 pt-2 text-sm sm:text-base md:text-lg"><li class="${
    escape(null_to_empty(`mr-4 ${"active-tab"}`), true) + " svelte-18fkfyi"
  }"><button class="${
          escape(null_to_empty(`p-2 ${"text-white"}`), true) + " svelte-18fkfyi"
        }">Fixtures
          </button></li>
        ${``}
        <li class="${
          escape(null_to_empty(`mr-4 ${""}`), true) + " svelte-18fkfyi"
        }"><button class="${
          escape(null_to_empty(`p-2 ${"text-gray-400"}`), true) +
          " svelte-18fkfyi"
        }">Leaderboards
          </button></li>
        <li class="${
      ...
      ${`${validate_component(Fixtures, "FixturesComponent").$$render(
        $$result,
        {},
        {},
        {}
