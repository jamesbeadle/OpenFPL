...
import {
  a as convertDateToReadable,
  b as getFlagComponent,
  c as calculateAgeFromNanoseconds,
  g as getPositionText,
  u as updateTableData,
} from "../../../chunks/team-store.js";
const ShirtIcon = create_ssr_component(
  ($$result, $$props, $$bindings, slots) => {
    ...
    if (
      $$props.primaryColour === void 0 &&
      $$bindings.primaryColour &&
      primaryColour !== void 0
    )
      $$bindings.primaryColour(primaryColour);
    if (
      $$props.secondaryColour === void 0 &&
      $$bindings.secondaryColour &&
      secondaryColour !== void 0
    )
      $$bindings.secondaryColour(secondaryColour);
    if (
      $$props.thirdColour === void 0 &&
      $$bindings.thirdColour &&
      thirdColour !== void 0
    )
...
const Player_gameweek_history = create_ssr_component(
  ($$result, $$props, $$bindings, slots) => {
    ...
    $$unsubscribe_page();
    return `${`${validate_component(LoadingIcon, "LoadingIcon").$$render(
      $$result,
      {},
      {},
      {}
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${validate_component(Layout, "Layout").$$render(
    ...
    {
      default: () => {
        return `<div class="m-4"><div class="flex flex-col md:flex-row"><div class="flex justify-start items-center text-white space-x-4 flex-grow m-4 bg-panel p-4 rounded-md"><div class="flex-grow flex flex-col items-center"><p class="text-gray-300 text-xs">${escape(
          getPositionText(-1)
        )}</p>
          <div class="py-2 flex">${validate_component(
            ShirtIcon,
            "ShirtIcon"
          ).$$render(
            $$result,
          ...
          <p class="text-gray-300 text-xs">Shirt: ${escape(
            selectedPlayer?.shirtNumber
          )}</p></div>
        <div class="flex-shrink-0 w-px bg-gray-400 self-stretch" style="min-width: 2px; min-height: 50px;"></div>
        <div class="flex-grow"><p class="text-gray-300 text-xs">${escape(
          team?.name
        )}</p>
          <p class="text-2xl sm:text-3xl md:text-4xl mt-2 mb-2 font-bold">${escape(
            selectedPlayer?.lastName
          )}</p>
          <p class="text-gray-300 text-xs flex items-center">${validate_component(
            getFlagComponent("") || missing_component,
            "svelte:component"
          ).$$render(
            $$result,
            { class: "w-4 h-4 mr-1", size: "100" },
            {},
            {}
          )}${escape(selectedPlayer?.firstName)}</p></div>
        <div class="flex-shrink-0 w-px bg-gray-400 self-stretch" style="min-width: 2px; min-height: 50px;"></div>
        <div class="flex-grow"><p class="text-gray-300 text-xs">Value</p>
          <p class="text-2xl sm:text-3xl md:text-4xl mt-2 mb-2 font-bold">£${escape(
            (Number(0) / 4).toFixed(2)
          )}m
          </p>
        ...
        <div class="flex-grow mb-4 md:mb-0"><p class="text-gray-300 text-xs">Age</p>
          <p class="text-2xl sm:text-3xl md:text-4xl mt-2 mb-2 font-bold">${escape(
            calculateAgeFromNanoseconds(Number(0))
          )}</p>
          <p class="text-gray-300 text-xs">${escape(
            convertDateToReadable(Number(0))
          )}</p></div></div>
      <div class="flex flex-col md:flex-row justify-start md:items-center text-white space-x-0 md:space-x-4 flex-grow m-4 bg-panel p-4 rounded-md"><div class="flex-grow mb-4 md:mb-0"><p class="text-gray-300 text-xs">Next Game:</p>
          <div class="flex justify-center mb-2 mt-2"><div class="flex justify-center items-center"><div class="w-10 ml-4 mr-4"><a${add_attribute(
            "href",
            `/club?id=${-1}`,
            0
          ...
          {}
              ...
              <div class="w-10 ml-4"><a${add_attribute(
                "href",
                `/club?id=${-1}`,
                0
              )}>${validate_component(BadgeIcon, "BadgeIcon").$$render(
          ...
          <div class="flex justify-center"><div class="w-10 ml-4 mr-4"><p class="text-gray-300 text-xs text-center"><a class="text-gray-300 text-xs text-center"${add_attribute(
            "href",
            `/club?id=${-1}`,
            0
          )}>${escape("")}</a></p></div>
            <div class="w-v ml-2 mr-2"></div>
            <div class="w-10 ml-4"><p class="text-gray-300 text-xs text-center"><a class="text-gray-300 text-xs text-center"${add_attribute(
              "href",
              `/club?id=${-1}`,
              0
            )}>${escape("")}</a></p></div></div></div>
        <div class="h-px bg-gray-400 w-full md:w-px md:h-full md:self-stretch" style="min-height: 2px; min-width: 2px;"></div>
        <div class="flex-grow mb-4 md:mb-0"><p class="text-gray-300 text-xs mt-4 md:mt-0">Kick Off:</p>
          <div class="flex"><p class="text-2xl sm:text-3xl md:text-4xl mt-2 mb-2 font-bold">${escape(
            countdownDays
          )}<span class="text-gray-300 text-xs ml-1">d</span>
              : ${escape(
                countdownHours
              )}<span class="text-gray-300 text-xs ml-1">h</span>
              : ${escape(
                countdownMinutes
              )}<span class="text-gray-300 text-xs ml-1">m</span></p></div>
          <p class="text-gray-300 text-xs">${escape(
            nextFixtureDate
          )} | ${escape(nextFixtureTime)}</p></div></div></div></div>

  <div class="m-4"><div class="bg-panel rounded-md m-4"><ul class="flex bg-light-gray px-4 pt-2"><li${add_attribute(
    "class",
    `mr-4 text-xs md:text-base ${"active-tab"}`,
    0
  )}><button${add_attribute("class", `p-2 ${"text-white"}`, 0)}>Gameweek History
          </button></li></ul>
      ${`${validate_component(
