...
const Player_events_modal = create_ssr_component(
  ($$result, $$props, $$bindings, slots) => {
    ...
    if (
      $$props.fixtureId === void 0 &&
      $$bindings.fixtureId &&
      fixtureId !== void 0
    )
      $$bindings.fixtureId(fixtureId);
    if (
      $$props.playerEventData === void 0 &&
      $$bindings.playerEventData &&
      playerEventData !== void 0
    )
      $$bindings.playerEventData(playerEventData);
    $$unsubscribe_playerEventData();
    return `${
      show
        ? `<div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full"><div class="relative top-20 mx-auto p-5 border w-3/4 shadow-lg rounded-md">${
            player
              ? `<div class="flex justify-between items-center"><h4 class="text-lg font-bold">${escape(
                  player.firstName !== ""
        ...
        <div class="mt-4 p-4 border-t border-gray-200"><h4 class="text-lg font-bold mb-3">Add Event</h4>
          <div class="flex flex-col gap-3"><div><label for="eventType" class="block text-sm font-medium text-gray-700">Event Type</label>
              <select id="eventType" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md"><option value="-1" disabled>Select event type</option>${each(
                eventOptions,
                (option) => {
            ...
            <div><label for="startMinute" class="block text-sm font-medium text-gray-700">Start Minute</label>
              <input type="number" id="startMinute" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md" placeholder="Enter start minute"${add_attribute(
                "value",
                eventStartTime,
                0
              )}></div>
            <div><label for="endMinute" class="block text-sm font-medium text-gray-700">End Minute</label>
              <input type="number" id="endMinute" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md" placeholder="Enter end minute"${add_attribute(
                "value",
                eventEndTime,
                0
              )}></div>
        ...
        <div class="mt-4"><ul class="list-disc pl-5">${each(
          $playerEventData,
          (event, index) => {
            return `<li class="flex justify-between items-center mb-2"><span>${escape(
              event.eventType
            )} - From ${escape(event.eventStartMinute)} to ${escape(
...
const Select_players_modal = create_ssr_component(
  ($$result, $$props, $$bindings, slots) => {
    let $teamPlayers, $$unsubscribe_teamPlayers;
    let { teamPlayers = writable([]) } = $$props;
    $$unsubscribe_teamPlayers = subscribe(
      teamPlayers,
    ...
    let { show = false } = $$props;
    if (
      $$props.teamPlayers === void 0 &&
      $$bindings.teamPlayers &&
      teamPlayers !== void 0
    )
      $$bindings.teamPlayers(teamPlayers);
    if (
      $$props.selectedTeam === void 0 &&
      $$bindings.selectedTeam &&
      selectedTeam !== void 0
    )
      $$bindings.selectedTeam(selectedTeam);
    if (
      $$props.selectedPlayers === void 0 &&
      $$bindings.selectedPlayers &&
      selectedPlayers !== void 0
    )
    ...
    return `${
      show
        ? `<div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full"><div class="relative top-20 mx-auto p-5 border w-3/4 shadow-lg rounded-md"><div class="flex justify-between items-center"><h4 class="text-lg font-bold">Select ${escape(
            selectedTeam.friendlyName
          )} Players
        </h4>
        <button class="text-black">✕</button></div>
      <div class="my-5 flex flex-wrap">${each($teamPlayers, (player) => {
        let selected = get_store_value(selectedPlayers).some(
          (p) => p.id === player.id
        );
        return `
          <div class="flex-1 sm:flex-basis-1/2"><label class="block"><input type="checkbox" ${
            selected ? "checked" : ""
          }>
              ${escape(
                `${
...
const Confirm_fixture_data_modal = create_ssr_component(
  ($$result, $$props, $$bindings, slots) => {
    ...
    return `${
      show
        ? `<div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full"><div class="relative top-20 mx-auto p-5 border w-3/4 shadow-lg rounded-md"><div class="flex justify-between items-center"><h4 class="text-lg font-bold">Confirm Fixture Data</h4>
        <button class="text-black">✕</button></div>
...
const Clear_draft_modal = create_ssr_component(
  ($$result, $$props, $$bindings, slots) => {
    ...
    return `${
      show
        ? `<div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full"><div class="relative top-20 mx-auto p-5 border w-3/4 shadow-lg rounded-md"><div class="flex justify-between items-center"><h4 class="text-lg font-bold">Confirm Clear Draft</h4>
        <button class="text-black">✕</button></div>
  ...
  async function confirmFixtureData() {
    ...
    try {
      await governanceStore.submitFixtureData(
        fixtureId,
        get_store_value(playerEventData)
      );
  ...
  return `${validate_component(Layout, "Layout").$$render(
    ...
    {},
    {
      default: () => {
        return `${
          isLoading
            ? `<div class="flex items-center justify-center h-screen"><p class="text-center mt-1">Loading Fixture Data...</p></div>`
            : `<div class="m-4"><button class="fpl-button">Save Draft</button>
      <div class="bg-panel rounded-lg m-4"><ul class="flex rounded-t-lg bg-light-gray px-4 pt-2"><li${add_attribute(
        "class",
        `mr-4 text-xs md:text-base ${"active-tab"}`,
        0
      )}><button${add_attribute("class", `p-2 ${"text-white"}`, 0)}>${escape(
                getTeamFromId(0)?.friendlyName
              )}</button></li>
          <li${add_attribute(
            "class",
            `mr-4 text-xs md:text-base ${""}`,
            0
          )}><button${add_attribute(
                "class",
                `p-2 ${"text-gray-400"}`,
                0
              )}>${escape(getTeamFromId(0)?.friendlyName)}</button></li></ul>

        ${`${each(
          $selectedPlayers.filter((x) => x.teamId === fixture?.homeTeamId),
          (player) => {
            return `<div class="card player-card mb-4"><div class="card-header"><h5>${escape(
              player.lastName
            )}</h5>
                <p class="small-text mb-0 mt-0">${escape(
                  player.firstName
                )}</p></div>
              <div class="card-body"><p>Events: ${escape(
                $playerEventData.filter((pe) => pe.playerId === player.id)
                  .length
              )}</p>
