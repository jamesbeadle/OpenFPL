...
module {
    ...
    func rec(t : Trie<K, V>, bitpos : ?Hash.Hash, bits : Hash.Hash, mask : Hash.Hash) : Bool {
      switch t {
        ...
        case (#leaf(l)) {
          ...
          ((len <= MAX_LEAF_SIZE) or (not enforceNormal)) and len == l.size and (
            List.all<(Key<K>, V)>(
              l.keyvals,
        ...
        case (#branch(b)) {
          ...
          ) and rec(b.left, ?bitpos1, bits, mask1) and rec(b.right, ?bitpos1, bits1, mask1);
        };
      };
    };
    rec(t, null, 0, 0);
  ...
  module ListUtil {
    ...
    public func lenIsEqLessThan<T>(l : List<T>, i : Nat) : Bool {
      switch l {
        case null { true };
        case (?(_, t)) {
          if (i == 0) { false } else { lenIsEqLessThan<T>(t, i - 1) };
        };
      };
    };
  ...
  public func replace<K, V>(t : Trie<K, V>, k : Key<K>, k_eq : (K, K) -> Bool, v : ?V) : (Trie<K, V>, ?V) {
    ...
    func rec(t : Trie<K, V>, bitpos : Nat) : (Trie<K, V>, ?V) {
      switch t {
        ...
        case (#leaf(l)) {
          let (kvs2, old_val) = AssocList.replace<Key<K>, V>(l.keyvals, k, key_eq, v);
  ...
  public func get<K, V>(t : Trie<K, V>, k : Key<K>, k_eq : (K, K) -> Bool) : ?V = find<K, V>(t, k, k_eq);
  ...
  public func prod<K1, V1, K2, V2, K3, V3>(
    ...
    func merge(a : Trie<K3, V3>, b : Trie<K3, V3>) : Trie<K3, V3> = mergeDisjoint<K3, V3>(a, b, k3_eq);
