...
let findTest = do {
  ...
  let actual : ?Element = Array.find<Element>(
    xs,
    func(x : Element) : Bool {
      x.key == "b";
    },
  ...
  let elementTestable : T.Testable<Element> = {
    display = func(e : Element) : Text {
      "{ key = " # T.textTestable.display(e.key) # ";" # " value = " # T.intTestable.display(e.value) # " }";
    };
    equals = func(e1 : Element, e2 : Element) : Bool = e1.key == e2.key and e1.value == e2.value;
  };
