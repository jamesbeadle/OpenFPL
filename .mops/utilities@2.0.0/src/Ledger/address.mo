...
module {
  ...
  public func from_identifier(aid : T.AccountIdentifier) : T.Address {
    ...
    if (not equal_arrays<Nat8>(crcx, crcy, func(x : Nat8, y : Nat8) = x == y)) trap("Account.from_identifier()") else Hex.encode(b);
  };
  ...
  public func to_identifier(aid : T.Address) : T.AccountIdentifier {
    switch (Hex.decode(aid)) {
      ...
      case (#ok(b)) {
        if (b.size() != 32) trap("Account.to_identifier()") else {
          let crcx : [Nat8] = byte_range(b, 0, 3);
          let crcy : [Nat8] = crc32(byte_range(b, 4, 31).vals());
          if (not equal_arrays<Nat8>(crcx, crcy, func(x : Nat8, y : Nat8) = x == y)) trap("Account.to_identifier()") else blobFromArray(b);
        };
      };
    };
  };
  ...
  public func is_valid(aid : T.Address) : Bool {
    switch (Hex.decode(aid)) {
      case (#err(_)) false;
      case (#ok(b)) {
        if (b.size() != 32) false else {
          let crcx : [Nat8] = byte_range(b, 0, 3);
          let crcy : [Nat8] = crc32(byte_range(b, 4, 31).vals());
          if (not equal_arrays<Nat8>(crcx, crcy, func(x : Nat8, y : Nat8) = x == y)) false else true;
        };
      };
    };
  };
