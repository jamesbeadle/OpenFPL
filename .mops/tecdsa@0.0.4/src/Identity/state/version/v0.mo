...
module {
  ...
  public type InitParams = {
    client : T.Client;
    key_id : T.KeyId;
    seed_phrase : ?T.SeedPhrase;
  };

  public func init(params : InitParams) : async * T.AsyncReturn<(T.SeedPhrase, State)> {
    let ?tag = KeyId.toTag(params.key_id) else {
      return #err(#other("mo:tecdsa/identity/state: line 15"));
    };
    ...
    let client_params : T.Params = {
      key_id = params.key_id;
      canister_id = null;
      derivation_path = [seed];
    };
    switch (await * params.client.request_public_key(client_params)) {
      ...
      case (#ok pk) {
        ...
        #ok(
          ...
          blobFromArray(
            tabulate<Nat8>(
              nat64ToNat(STATE_SIZE),
              func(i) : Nat8 {
                if (i == 0) tag.0 else if (i == 1) tag.1 else if (i < 90) pk[i -2] else seed_bytes[i - 90];
              },
            ),
          ),
        );
