...
module {
  ...
  public func generate(size : Nat, generator : GenFn) : Int {
    func evaluate(num : Nat, byte : Nat8, bit : Nat8) : Nat {
      if (btstNat8(byte, bit)) if (bit != 0x00) evaluate(num * 2 +1, byte, bit -1) else num * 2 + 1 else if (bit != 0x00) evaluate(num * 2, byte, bit -1) else num * 2;
    };
    func sum(num : Nat, cnt : Nat) : Nat {
      if (cnt == size) num else sum(
