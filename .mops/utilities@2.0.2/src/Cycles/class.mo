...
module {
  ...
  public func constructor(state_ : T.State) : T.Cycles = object {
    ...
    public func reserve(amt : Nat) : Bool {
      ...
      if (bal <= reserved) false else if ((bal - reserved) < state.minimum_balance) false else {
    ...
    public func transfer(request : T.TransferRequest) : async * T.Return<Nat> {
      ...
      let amt_req = List.foldLeft<(Any, Nat), Nat>(
        ...
        0,
        func(x, (_, y)) : Nat {
          if (y >= request.max_cycles) 0 else request.max_cycles - y;
        },
      );
    ...
    public func status(request : T.StatusRequest) : async * T.ReturnStatus {
      ...
      return {
        ...
        trapped = Buffer.toArray<Text>(trapped);
      }
    ...
    public func change(request : T.ChangeRequest) : async * T.Return<()> {
      ...
      for (arg in request.args.vals()) {
        ...
        switch arg {
          ...
          case (#AddClient prin) {
            ...
            state.clients := tabulate<Principal>(
              size,
              func(i) {
          ...
          case (#AddAdmin prin) {
            ...
            state.admins := tabulate<Principal>(
              size,
              func(i) {
      ...
      if (client_args.size() > 0) {
        ...
        let client_request : T.ChangeRequest = {
          args = Buffer.toArray<T.ChangeRequestArg>(client_args);
        };
