...
module {
  ...
  public func loadPreG(
    ...
    let _leArrayToNat32 = func(ofs : Nat) : Nat32 {
      return Nat32.fromNat(Nat8.toNat(arr[ofs +0])) | Nat32.fromNat(Nat8.toNat(arr[ofs +1])) << 8 | Nat32.fromNat(Nat8.toNat(arr[ofs +2])) << 16 | Nat32.fromNat(Nat8.toNat(arr[ofs +3])) << 24;
    };

    let res = Array.tabulate<AffineStorage>(
      ECMULT_TABLE_SIZE_G,
      func(i) {
        let offset = i * 2 * 8 * 4;
        let af = Group.AffineStorage();
        af.x.n := Array.tabulateVar<Nat32>(
          8,
          func(j) {
            _leArrayToNat32(offset + ((8 -1) * 4) - (j * 4));
          },
        );
        af.y.n := Array.tabulateVar<Nat32>(
          8,
          func(j) {
  ...
  func _calcPreC() : [[AffineStorage]] {
    ...
    let prec = Array.tabulate<[var AffineStorage]>(
      64,
      func i = Array.tabulateVar<AffineStorage>(
        16,
        func i = Group.AffineStorage(),
      ),
    );
