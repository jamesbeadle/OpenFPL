...
module {
  public class PublicKey(
    ...
    public func serialize_compressed() : [Nat8] {
  ...
  public func parse(
    ...
    if (
      not (
        p[0] == Utils.TAG_PUBKEY_FULL or p[0] == Utils.TAG_PUBKEY_HYBRID_EVEN or p[0] == Utils.TAG_PUBKEY_HYBRID_ODD,
      ),
    ) {
    ...
    elem.set_xy(x, y);
    if (
      (p[0] == Utils.TAG_PUBKEY_HYBRID_EVEN or p[0] == Utils.TAG_PUBKEY_HYBRID_ODD) and (y.is_odd() != (p[0] == Utils.TAG_PUBKEY_HYBRID_ODD)),
    ) {
      return #err(#InvalidPublicKey);
    };
    if (elem.is_infinity()) {
