...
module {
  ...
  public class Client(state_ : S.State, transformFn : H.TransformFunction) = {
    ...
    public func calculate_fee(request_bytes : Nat64, response_bytes : ?Nat64) : T.ReturnFee {
      let max_response : Nat64 = switch (response_bytes) {
        case (?n64) n64;
        case null C.DEFAULT_MAX_RESPONSE_BYTES;
      };
      let #ok(base_fee) = fees.get(C.FEES.ID.PER_CALL) else {
        return #err(#fee_not_defined(C.FEES.ID.PER_CALL));
      };
      let #ok(request_fee) = fees.multiply(C.FEES.ID.PER_REQUEST_BYTE, request_bytes) else {
        return #err(#fee_not_defined(C.FEES.ID.PER_REQUEST_BYTE));
      };
      let #ok(response_fee) = fees.multiply(C.FEES.ID.PER_RESPONSE_BYTE, max_response) else {
        return #err(#fee_not_defined(C.FEES.ID.PER_RESPONSE_BYTE));
      };
              ...
              await ic.http_request({
                ...
                headers = [
                  { name = "Content-Type"; value = "application/cbor" },
                  {
                    ...
                    value = nonce_factory.next_string();
                  },
                ];
              });
    ...
    func process_http_response(res : H.HttpResponsePayload) : T.Response {
      ...
      let ?status = content.get<Text>("status", Cbor.getText) else {
        return #err(#missing("status field"));
      };
      if (status == "processing") #ok(#unknown) else if (status == "replied") {
        let ?response = content.get<[Nat8]>("response", Cbor.getBytearray) else {
        ...
        let ?error_message = content.get<Text>("error", Cbor.getText) else {
          return #err(#missing("error message"));
        };
