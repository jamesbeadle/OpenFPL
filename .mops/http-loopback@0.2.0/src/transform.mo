...
module {
        ...
        switch (check_rejected(content)) {
          ...
          case null switch (check_query(content)) {
            case (?(status, response)) {
              if (response.size() == 0) respond_error(status) else respond_status(status, response);
            };
            case null switch (check_read_state(content, args.context)) {
              case (?(status, response)) {
                if (response.size() == 0) {
                  if (status != "processing") respond_error(status) else respond_status(status, response);
                ...
                switch (decodeUtf8(encoded_status)) {
                  case (?status) {
                    if (status == "replied") {
                      ...
                      let ?candid = hashtree.lookup(res_path) else {
                        return ?("missing canister response", []);
                      };
                      ?(status, blobToArray(candid));
                    } else if (status == "rejected") {
                      let msg_path = [encodeUtf8("request_status"), reqid, encodeUtf8("reject_message")];
                      let ?reject_message = hashtree.lookup(msg_path) else {
                        return ?("missing reject_message", []);
                      };
  ...
  func check_rejected(content : Cbor.ContentMap) : ?(Text, [Nat8]) {
    let ?reject_code = content.get<Nat64>("reject_code", Cbor.getNat64) else {
      return null;
    };
    let ?reject_message = content.get<Text>("reject_message", Cbor.getText) else {
      return null;
    };
  ...
  func check_query(content : Cbor.ContentMap) : ?(Text, [Nat8]) {
    switch (content.get<Text>("status", Cbor.getText)) {
      ...
      case (?status) {
        if (status == "replied") {
          let ?map = content.get<Cbor.CborMap>("reply", Cbor.getMap) else {
            return ?("missing query reply", []);
          };
          let response = Cbor.populated(Cbor.from_cbor_map(map));
          let ?arg = response.get<[Nat8]>("arg", Cbor.getBytearray) else {
            return ?("missing query reply arg", []);
          };
          ?(status, arg);
        } else if (status == "rejected") {
          let ?reject_message = content.get<Text>("reject_message", Cbor.getText) else {
