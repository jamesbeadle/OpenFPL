...
module {
  ...
  func to_message_id(hash : [Nat8]) : Blob {
    Blob.fromArray(
      tabulate<Nat8>(
        43,
        func(i) = if (i < 11) IC_REQUEST_DOMAIN_SEPERATOR[i] else hash[i - 11],
      ),
    );
  };
    ...
    switch (req.request) {
      ...
      case (#read_state params) {
        ...
        content.set(
          "paths",
          #majorType4(
            mapEntries<[Blob], T.CborArray>(
              params.paths,
              func(state_path, _) : T.CborArray {
                #majorType4(
                  mapEntries<Blob, T.CborBytes>(
                    state_path,
                    func(path_label, _) : T.CborBytes {
    ...
    switch (req.request) {
      ...
      case (#read_state params) {
        ...
        buffer.add((
          "paths",
          #Array(
            mapEntries<[Blob], Hash.Value>(
              params.paths,
              func(state_path, _) : Hash.Value {
                #Array(
                  mapEntries<Blob, Hash.Value>(
                    state_path,
                    func(path_label, _) : Hash.Value {
                      #Blob(path_label);
                    },
                  ),
                );
